...
Python automation script for analyzing the QP (Quantization Parameter) feature of the x264 encoder.

The script runs the encoder using QP values from 1 to 51 on an uncompressed .yuv video and generates reports demonstrating how QP affects compression efficiency.
...
This project shows how encoder parameters affect compression behavior.
...
Download a prebuilt binary from a trusted source.

Extract the archive.

Add the folder containing x264.exe to your PATH environment variable.
...
Run the script from the project directory while the virtual environment is active.
...
Runs the encoder with QP values from 1 to 51

Generates encoded .264 files

Measures encoding metrics
